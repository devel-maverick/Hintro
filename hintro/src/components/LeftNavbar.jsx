...
function LeftNavbar({ activeTab, setActiveTab, onOpenFeedbackModal, isSidebarOpen, onCloseSidebar }) {
  return (
    <>

    {isSidebarOpen && (
      <div className='fixed inset-0 bg-black/30 z-40 md:hidden' onClick={onCloseSidebar} />
    )}
    <div className={`w-[260px] flex flex-col border-r border-gray-200 min-h-screen shrink-0 bg-white
      fixed md:static z-50 md:z-auto transition-transform duration-300 ease-in-out
      ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
    `}>
      <div className='flex items-center justify-between md:justify-center border-b border-gray-200 h-[65px] shrink-0 w-full shadow-sm z-10 px-4 md:px-0'>
        <button className='md:hidden text-gray-500 hover:text-gray-700' onClick={onCloseSidebar}>
          <X className='w-5 h-5' />
        </button>
