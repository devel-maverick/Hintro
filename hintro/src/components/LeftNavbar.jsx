...
const navItems = [
  ...
  { icon: CallInsightsIcon, label: 'Call Insights' },
  { icon: KnowledgeBaseIcon, label: 'Knowledge Base', hasInfo: true },
  { icon: PromptsIcon, label: 'Prompts', hasInfo: true },
  { icon: BoxyControlsIcon, label: 'Boxy Controls', hasInfo: true },
]

function LeftNavbar() {
  const [activeNav, setActiveNav] = useState('Dashboard')
  return (
        <div className='w-[260px] flex flex-col border-r border-gray-200 min-h-screen'>
            <div className='flex items-center justify-center border-b border-gray-200 h-[65px] drop-shadow'>
                <h1 className='text-[24px] text-black/80 ml-3'>Hintro</h1>
            </div>
            <div className='flex flex-col flex-1'>
                <div className='w-full flex flex-col gap-2 py-4 px-4 '>
                    {navItems.map(({ icon: Icon, label, hasInfo }) => (
                      <div
                        key={label}
